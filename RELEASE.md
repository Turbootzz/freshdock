...
2. Create the approval gate: Settings, then Environments, then **New environment**
   named `release`, then add yourself under **Required reviewers**. Without this, the
   `publish` job would run unattended.
3. Let release-plz open PRs: Settings, then Actions, then General, then **Workflow
   permissions**, then "Read and write permissions" **and** check "Allow GitHub Actions
...
1. Watch `build`, `image`, `package-check`, and `release` run automatically in that order.
